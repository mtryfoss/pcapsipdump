...
void Trigger::add(const string s)
{
    ...
    {
        cout << "ERROR: Can't make sense of trigger '" << s << "': Not a valid trigger condition '" << trigger << "'." << endl;
        exit(2);
    }
    if (action == "mv")
    {
    ...
    {
        cout << "ERROR: Can't make sense of trigger '" << s << "': Unrecognized action: '" << action << "'" << endl;
    };
