    ...
    {
        {
            ...
            if (header_ip->version == 4 && (header_ip->frag_off & htons(0x1fff)) > 0)
            {
                ...
                if (f)
                {
                    // Happy path: call already known, write directly
                ...
                {
                    // Call not known yet — buffer for later
                    ct->buffer_frag(aai, pkt_header, pkt_data);
                }
            }
            ...
            )
            {
                         ...
                         !memcmp(data, "SIP/2.0 ", strlen("SIP/2.0 ")))
                {
                    ...
                    if (idx >= 0)
                    {
                        ...
                        // In the if (idx >= 0) block, after pcap_dump() writes the current packet:
